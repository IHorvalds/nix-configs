...
alias bbccc="bitbake -c clean -c cleanall -c cleansstate"

get_d7_sdk() {
	local dest=${1:-.}
	URL=$(curl -s d7-builds:8000 | jq -r '.sdk_root | to_entries | max_by(.key) | .value | map(select(.url | contains("linux")).url) | sort | reverse | first')
	echo "${dest}/${URL##*/}"
	(cd "${dest}" && curl -O "$URL")
}
...
install_d7_sdk() {
	...
	(
		...
		./installer-linux.sh $eula "${HOME}/Tools/SDKs/d7-sdk-${version}"
	)
}
